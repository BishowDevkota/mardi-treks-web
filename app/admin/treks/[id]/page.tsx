...
export default async function EditTrekPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const trek = await prisma.trek.findUnique({
    where: { id },
    include: { highlights: { orderBy: { sort: "asc" } }, itinerary: { orderBy: { dayNumber: "asc" } }, pricingTiers: true, availableDates: true, faqs: true, reviews: true },
  });
  if (!trek) notFound();
  return (
    <div>
