...
export async function sendBookingNotification({
  ...
  const addonsRows = addons && addons.length > 0
    ? addons.map((a) => `<tr><td style="padding:6px 8px;border-bottom:1px solid #eee;">${a.title} &times; ${a.qty}</td><td style="padding:6px 8px;border-bottom:1px solid #eee;text-align:right;">+$${(a.qty * a.pricePerUnit).toLocaleString()}</td></tr>`).join("")
    : "";
  ...
  const travelerRows = travelers.map((t, i) => `
    ...
    <tr><td style="padding:4px 8px;color:#64748b;width:120px;">Name</td><td style="padding:4px 8px;">${t.fullName}</td></tr>
    ...
    html: `
      <div style="font-family:sans-serif;max-width:600px;margin:0 auto;">
        ...
        <table style="width:100%;border-collapse:collapse;margin-top:12px;">
          <tr><td style="padding:8px;font-weight:bold;color:#46576a;width:120px;">Customer</td><td style="padding:8px;">${customerName}</td></tr>
          <tr><td style="padding:8px;font-weight:bold;color:#46576a;">Email</td><td style="padding:8px;">${customerEmail}</td></tr>
          <tr><td style="padding:8px;font-weight:bold;color:#46576a;">Trek</td><td style="padding:8px;">${trekTitle}</td></tr>
          <tr><td style="padding:8px;font-weight:bold;color:#46576a;">Start Date</td><td style="padding:8px;">${startDate}</td></tr>
        ...
        <p style="color:#334155;background:#fef3c7;padding:12px;border-radius:8px;">${specialRequests}</p>
        ` : ""}
